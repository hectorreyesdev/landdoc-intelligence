---
description: Wrap a build session — clarify any ambiguous curation calls, append to wiki/logs/, capture wiki/notes/, log corrections to tasks/lessons.md, drift-check (never edit) the living wiki/docs/, flag missing judgment artifacts, then confirm before committing only what I authored.
...
> **`wiki/logs/` = what happened · `wiki/docs/` = how the app works (living) · `wiki/notes/` = what I know · `tasks/lessons.md` = what broke + the rule.**
...
> **Authors + commits:** `wiki/logs/`, `wiki/notes/`, `tasks/lessons.md` — narrative, append-only, low-judgment.
> **NEVER writes/commits/regenerates:** `wiki/docs/*` (PRD · STACK · ARCHITECTURE · DATA-MODEL · DATA-FLOW · API · RUNBOOK · GLOSSARY) **and** `wiki/docs/decisions/` (ADRs) and their Mermaid. For these, /wrap **drift-detects and flags only** — the human edits and stages them.

## 0. Orient
- Project root = the git repo root containing the cwd. Work there. **If `wiki/` or `specs/` don't
  exist yet, run `/wiki-init` first** (this command depends on that tree).
- Read the latest `wiki/logs/<date>.md`, `tasks/lessons.md`, and `git log` / `git diff` since the
...
- /wrap curates "what happened" — most of it is factual and derivable from git + the session. But a
  few calls are genuine judgment: was an exchange a **real correction** worth a `tasks/lessons.md`
  line, or just normal iteration? is a finding **durable/reusable** enough for a note, or does it
  stay in the log? does a decision rise to a **significant decision that needs an ADR**? how should
  the session be segmented into topics?
...
- This step never authors `wiki/docs`/ADRs — clarifying a drift or a missing artifact still ends in a
  **flag** (steps 5–6), not an edit.

## 2. Log → `wiki/logs/<YYYY-MM-DD>.md` (ADDITIVELY — never duplicate)
...
Evidence: [[specs/<NNNN-feature>]] · [[wiki/docs/decisions/NNNN-<title>]] · PR #<n> / <branch>
...
## 4. Corrections → `tasks/lessons.md` (only REAL ones, newest at the bottom)
- For every correction I made this session — you did X, I told you to do Y instead — append one
  line **at the bottom** of `tasks/lessons.md` (newest last, matching the file's stated
  convention): `[YYYY-MM-DD] | what went wrong | rule to follow next time`.
- ONLY genuine corrections from this session. **Never manufacture entries** to look thorough; if
  there were no corrections, write nothing and say so.

## 5. Living docs → `wiki/docs/` — DRIFT-DETECT and FLAG ONLY (do NOT edit)
...
- Output format — a flag list, e.g.:
  `- DRIFT · wiki/docs/STACK.md §Backend runtime — claims .NET 9; csproj now targets net10.0 → you update`
- **Do not** write to any `wiki/docs/*` file, do not author ADR Decision/Consequences bodies, and
...
- A **significant decision with no ADR** under `wiki/docs/decisions/`.
- An **implementation with no preceding spec** in `specs/`, or **landed with no tests**.
- A **guardrail touched**: secrets handling, or `IChatClient`/`IEmbeddingClient` (interface or
  adapter wiring) changed **without a spec** in `specs/`.
...
- Stage ONLY what this command wrote: `git add wiki/logs wiki/notes tasks/lessons.md`. Do **not**
  `git add` any `wiki/docs/` path at all — any `wiki/docs/` file **I** already staged myself this
