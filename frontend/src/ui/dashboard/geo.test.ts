...
it('joins (state, county) counts to centroids, case-insensitively, carrying document ids', () => {
  const locations: readonly StateCountyCount[] = [
    { state: 'Texas', county: 'Reeves', count: 3, documentIds: ['a', 'b', 'c'] },
  ]
  expect(resolveMarkers(locations, index)).toEqual([
    { state: 'Texas', county: 'Reeves', count: 3, lon: -103.5, lat: 31.3, documentIds: ['a', 'b', 'c'] },
  ])
})
...
it('matches a "<County> County"-suffixed value against the bare us-atlas name', () => {
  ...
  const locations: readonly StateCountyCount[] = [
    { state: 'New Mexico', county: 'Eddy County', count: 2, documentIds: ['d1', 'd2'] },
  ]
  expect(resolveMarkers(locations, idx)).toEqual([
    { state: 'New Mexico', county: 'Eddy County', count: 2, lon: -104.3, lat: 32.5, documentIds: ['d1', 'd2'] },
  ])
})

it('drops counties the atlas cannot resolve', () => {
  const locations: readonly StateCountyCount[] = [
    { state: 'Atlantis', county: 'Nowhere', count: 5, documentIds: ['x'] },
  ]
  expect(resolveMarkers(locations, index)).toEqual([])
})
...
it('sorts markers largest-count-first so big bubbles render beneath small ones', () => {
  const locations: readonly StateCountyCount[] = [
    { state: 'New Mexico', county: 'Lea', count: 1, documentIds: ['a'] },
    { state: 'Texas', county: 'Reeves', count: 4, documentIds: ['b'] },
  ]
