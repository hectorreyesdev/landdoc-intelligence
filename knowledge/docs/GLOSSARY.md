...
- **Easy Auth** — Azure Container Apps built-in authentication: a platform sidecar that signs users in (Microsoft Entra ID here) before requests reach the app, injecting the authenticated principal as `X-MS-CLIENT-PRINCIPAL-*` headers; the gate of the single-user auth posture — see [ADR-0022](decisions/0022-single-user-entra-auth-easy-auth-gate-app-level-allowlist.md).
- **Principal / object ID** — the immutable Entra directory id of a signed-in identity; the value Easy Auth injects in `X-MS-CLIENT-PRINCIPAL-ID` and the app's `Auth:AllowedPrincipalIds` allowlist checks (not a secret).
