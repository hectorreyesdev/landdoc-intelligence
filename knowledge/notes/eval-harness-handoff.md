# Handoff — RAG Answer-Quality Eval Harness (spec 0006 / ADR-0018)

> Transient handoff note for continuing the eval-harness build in a fresh session. Safe to delete once
> the harness is complete. The authoritative design is **spec 0006** + **ADR-0018** (committed); this
...
## Locked decisions (see spec 0006 + ADR-0018)
...
- **Invocation:** local (`dotnet test` on the runner project) + a **manual** `eval.yml` CI job
  ...
  CI change yet. The `eval.yml` draft is in spec 0006 (and the plan file). Do not create
...
1. `docs(spec+adr)`: spec 0006 + ADR-0018 + propagation (README decisions index, specs index, STACK row).
   Commit subject starts `docs(spec+adr): spec 0006 RAG answer-quality eval harness ...`.
