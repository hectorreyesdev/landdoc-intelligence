...
Public repo, committed under my name. `knowledge/docs/*` and ADRs encode boundary decisions, so **only the
...
## ADR conventions
- Numbered `NNNN-slug` in `knowledge/docs/decisions/`; Status `Proposed | Accepted | Superseded by NNNN`.
- Every ADR is linked from the `knowledge/README.md` index (the canonical list) **and** ≥1 content doc.
